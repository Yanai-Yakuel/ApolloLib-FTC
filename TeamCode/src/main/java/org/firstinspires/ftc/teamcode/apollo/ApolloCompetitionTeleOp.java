...
public class ApolloCompetitionTeleOp extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        telemetry.addData("Clean IMU", "%.1f°", Math.toDegrees(
                imuClean.getRobotYawPitchRollAngles().getYaw(AngleUnit.RADIANS)));
        telemetry.addData("Drive IMU", "%.1f°", Math.toDegrees(drive.getRawExternalHeading()));
        telemetry.update();
    ...
    private void reset_pos(Pose2d currentPos) {
    ...
    private void sendTelemetry(Pose2d pose, boolean tagVisible, boolean aligning) {
        ...
        telemetry.addData("Clean IMU", "%.1f°", Math.toDegrees(cleanImu));
        telemetry.addData("Drive IMU", "%.1f°", Math.toDegrees(driveImu));
        telemetry.addData("Pose Head", "%.1f°", Math.toDegrees(poseHead));
        telemetry.addData("Delta Clean", "%.1f°", Math.toDegrees(Angle.normDelta(poseHead - cleanImu)));
